...
class SignUpVC: UIViewController {
    ...
    @objc func signUp() {
        AuthService.insance.createUser(withEmail: emailTextField.text!, andPassword: passwordTextField.text!, andUsername: nameTextField.text!) { success, error in
            if let error = error {
                self.showAlert(alertText: "Error", alertMessage: "Passwords must be at least 6 characters long.")
            } else {
                self.goToLoginVC()
                self.showAlert(alertText: "Success", alertMessage: "You have signed up successfully")
            }
        }
    }
