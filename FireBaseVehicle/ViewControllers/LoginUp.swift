...
class LoginVC: UIViewController {
    ...
    @objc func login() {
        ...
        AuthService.insance.loginUser(withEmail: emailTextField.text!, andPassword: passwordTextField.text!) { success, error in
            if let error = error {
                self.showAlert(alertText: "Error", alertMessage: "E-mail or Password is not correct")
            } else  {
                self.goToMainVC()
            }
        }
